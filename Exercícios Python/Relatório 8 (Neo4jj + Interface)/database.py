...
class GameDatabase:
    ...
    def update_player(self, player_id, new_name):
        with self._driver.session() as session:
            session.run("MATCH (p:Player {player_id: $player_id}) SET p.name = $new_name", player_id=player_id, new_name=new_name)
    ...
    def get_match_info(self, match_id):
        with self._driver.session() as session:
            result = session.run("MATCH (m:Match {match_id: $match_id})<-[:PARTICIPATED_IN]-(p:Player) "
                                 "RETURN p.player_id AS player_id, p.name AS name, m.result AS result", match_id=match_id)
